...
if ($productoId <= 0) {
    volverProducto('Producto invalido.');
}
...
if (!in_array($estado, ['activo', 'inactivo'], true)) {
    volverProducto('Estado de producto invalido.');
}
...
if (!$stmt->fetch()) {
    volverProducto('No se encontro el producto para actualizar.');
}

$stmt = $pdo->prepare('SELECT id FROM productos WHERE LOWER(TRIM(nombre)) = LOWER(TRIM(?)) AND id <> ? LIMIT 1');
$stmt->execute([$nombre, $productoId]);
if ($stmt->fetch()) {
    volverProducto('Ya existe otro producto con ese nombre.');
}

if ($codigoBarra !== '') {
    $stmt = $pdo->prepare('SELECT id FROM productos WHERE codigo_barra = ? AND id <> ? LIMIT 1');
    $stmt->execute([$codigoBarra, $productoId]);

    if ($stmt->fetch()) {
        volverProducto('Ya existe otro producto con ese codigo de barras.');
    }
}
...
$stmt = $pdo->prepare(
    'UPDATE productos
     SET nombre = ?, codigo_barra = ?, proveedor_id = ?, categoria = ?, precio_compra = ?, precio_venta = ?, pack_cantidad = ?, precio_compra_pack = ?, precio_pack = ?, promocion_cantidad = ?, precio_promocion = ?, stock = ?, estado = ?
     WHERE id = ?'
);
$stmt->execute([
    $nombre,
